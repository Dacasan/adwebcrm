...
export const RATE_LIMITS = {
  ...
  /** Subida de radiografías y presupuestos (`/api/uploads`), por IP.
   *  Más estrecho que cualquier otro bucket público porque cada hit
   *  escribe bytes en Storage: seis archivos de 20 MB son 120 MB por
   *  petición. 5/min deja a un paciente mandar su pano, darse cuenta de
   *  que faltaba el presupuesto y volver a enviar, y corta en seco a
   *  quien intente usar el bucket como disco duro gratis. */
  uploads: { limit: 5, windowMs: 60_000 },
  /** Webhooks de Twilio, por `webhook_token` (es decir, por cuenta).
   *  Una llamada normal genera del orden de 6-8 callbacks (initiated,
   *  ringing, in-progress, action, completed, recording) y un SMS otros
   *  2-3, así que 600/min deja tráfico real de decenas de llamadas
   *  simultáneas y sigue cortando un bucle. Va DESPUÉS de resolver el
   *  token y ANTES de verificar la firma: un atacante sin firma válida no
   *  puede consumir el presupuesto de otra cuenta, solo el del token que
   *  haya adivinado. */
  twilioWebhook: { limit: 600, windowMs: 60_000 },
  /** Webhook de SendGrid, por `webhook_token`. SendGrid agrupa eventos en
   *  lotes de hasta ~1 000, así que el número de PETICIONES es bajo aunque
   *  el volumen sea alto; 120/min es holgado y acota el reintento
   *  agresivo que SendGrid hace ante cualquier no-200. */
  sendgridWebhook: { limit: 120, windowMs: 60_000 },
} as const;

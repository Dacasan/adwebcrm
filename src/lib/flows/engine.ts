...
import { interpolateVars as interpolateVarsShared } from "@/lib/templates/interpolate";
...
/**
 * `{{vars.foo}}` interpolation (implementación compartida en
 * src/lib/templates/interpolate.ts — punto 3 consolidación Fase 6).
 * Used by send_message + collect_input prompt text so a captured
 * `name` can show up in the next prompt ("Thanks {{vars.name}}, what's
 * your email?"). Missing vars render as empty string.
 */
function interpolateVars(template: string, vars: Record<string, unknown>): string {
  return interpolateVarsShared(template, vars);
}

...
export default function PipelinesPage() {
  ...
  // Stage transition checklist modal (067): pausa la transición cuando la
  // etapa destino tiene checklist, para que el agente confirme ANTES de mover.
  ...
  const commitDealMove = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      // transition_deal es la ÚNICA vía de movimiento (DAD §7.1): optimistic
      // locking (version). El update directo de stage está prohibido. Va por la
      // ruta de servidor y no por la RPC directa porque el despacho del trigger
      // `deal_stage_changed` solo puede ocurrir en el servidor.
      const result = await transitionDeal(dealId, {
    ...
    [refreshDeals, t, deals],
  );

  // Etapa destino sin checklist → transición inmediata. Con checklist → el
  // modal de confirmación (067) pausa el movimiento hasta que el agente
  // confirme. El checklist NUNCA bloquea: el botón siempre está habilitado.
  const handleRequestDealMove = useCallback(
    (dealId: string, toStageId: string) => {
      const toStage = stages.find((s) => s.id === toStageId);
      if (!toStage || toStage.checklist.length === 0) {
        commitDealMove(dealId, toStageId);
        return;
      }
      setPendingMove({ dealId, toStageId });
      setTransitionModalOpen(true);
    },
    [stages, commitDealMove],
  );

  const handleConfirmDealMove = useCallback(() => {
    if (!pendingMove) return;
    const { dealId, toStageId } = pendingMove;
    setTransitionModalOpen(false);
    setPendingMove(null);
    commitDealMove(dealId, toStageId);
  }, [pendingMove, commitDealMove]);
      ...
      {pipelines.length === 0 ? (
        ...
        <>
          ...
          <PipelineBoard
            stages={stages}
            deals={deals}
            onRequestDealMove={handleRequestDealMove}
        ...
        fromStage={
          pendingMove
            ? (stages.find(
                (s) =>
                  s.id ===
                  deals.find((d) => d.id === pendingMove.dealId)?.stage_id,
              ) ?? null)
            : null
        }
        toStage={
          pendingMove
            ? (stages.find((s) => s.id === pendingMove.toStageId) ?? null)
            : null
        }
        onConfirm={handleConfirmDealMove}
      />
    </div>
  );
}

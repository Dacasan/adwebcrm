...
export async function POST(request: Request) {
  ...
  //
  // `requireRole` already resolves the user *and* the account in one round
  // trip, and `automations.account_id` is NOT NULL post-017, so we take
  // `accountId` from it rather than re-querying `profiles`.
  let userId: string

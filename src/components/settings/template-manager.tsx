...
import {
  ...
  TemplateFormDialog,
  TemplateFormNotice,
} from '@/components/ui/template-form';
import {
  Select,
  SelectContent,
  SelectItem,
  ...
  return (
    <section className="animate-in fade-in-50 space-y-4 duration-200">
      ...
      <TemplateFormDialog
        ...
        title={editingId ? t('dialogEditTitle') : t('dialogNewTitle')}
        description={editingId ? t('dialogEditDesc') : t('dialogNewDesc')}
        notice={
          form.category === 'Authentication' ? (
            <TemplateFormNotice>
              {t.rich('authWarning', { bold: (chunks) => <strong>{chunks}</strong> })}
            </TemplateFormNotice>
          ) : null
        }
        cancelLabel={t('cancel')}
        submitLabel={editingId ? t('saveResubmit') : t('submitApproval')}
        submittingLabel={editingId ? t('saving') : t('submitting')}
        submitting={submitting}
        submitDisabled={form.category === 'Authentication'}
        onSubmit={handleSubmit}
      >
        <TemplateField
          label={t('templateName')}
          hint={editingId ? t('nameFixed') : t('nameHint')}
        >
          <Input
        ...
        <TemplateFieldGrid>
          <TemplateField label={t('category')}>
            <Select
              value={form.category}
              onValueChange={(val) =>
                setForm({
          ...
          <TemplateField
            ...
            hint={
              editingId
                ? t('langFixed')
                : t.rich('langHint', { code: (chunks) => <code>{chunks}</code> })
            }
          >
            <Input
        ...
        <TemplateField label={t('bodyText')} hint={t('bodyHint')}>
          <Textarea

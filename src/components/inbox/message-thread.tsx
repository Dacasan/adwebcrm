...
/**
 * Canal por el que viajó (o viajará) un mensaje.
 *
 * El canal es una propiedad del MENSAJE, no del hilo: `conversations` no
 * tiene columna de canal y por diseño hay UNA conversación por (cuenta,
 * contacto), así que WhatsApp, SMS y email del mismo contacto caen en el
 * mismo hilo (migración 041: "mezcla solo visual en el inbox").
 */
export type ThreadChannel = "whatsapp" | "sms" | "email";

/**
 * `messages.channel` existe en BD desde la 041 (NOT NULL DEFAULT
 * 'whatsapp', ampliado a 'email' en la 078) y llega al cliente porque el
 * fetch de abajo hace `select("*")`. Lo leemos por intersección en vez de
 * tocar `src/types/index.ts`, que es de otro bloque: si `Message` acaba
 * declarando `channel`, esta intersección sigue siendo válida.
 */
type MessageWithChannel = Message & { channel?: ThreadChannel };

/**
 * Canal de un mensaje concreto. Las filas históricas y los payloads
 * optimistas que no lo traen son de WhatsApp — es el default de la
 * columna y el único canal que existía antes de la 041.
 */
function messageChannel(m: Message): ThreadChannel {
  return (m as MessageWithChannel).channel ?? "whatsapp";
}

const CHANNEL_ICONS: Record<ThreadChannel, typeof MessageCircle> = {
...
export function MessageThread({
  ...
  /**
   * Canal activo del hilo, derivado del último mensaje que lo declara.
   *
   * No hay columna de canal en `conversations` ni la va a haber: partir
   * los hilos por canal rompería la deduplicación por (cuenta, contacto)
   * de la que dependen los webhooks entrantes. Derivarlo del último
   * mensaje es lo que el agente ve arriba del todo y, sobre todo, por
   * donde el cliente espera la respuesta: si escribió un SMS, se le
   * contesta por SMS.
   *
   * Los mensajes optimistas que construyen los handlers de abajo también
   * llevan `channel`, así que enviar no hace bailar el canal derivado
   * mientras el INSERT real viaja por realtime.
   */
  const channel = useMemo<ThreadChannel>(() => {
    for (let i = messages.length - 1; i >= 0; i--) {
      const c = (messages[i] as MessageWithChannel).channel;
      if (c) return c;
    }
    return "whatsapp";
  }, [messages]);
  ...
  //
  // Es un concepto EXCLUSIVO de WhatsApp: en SMS no existe, y aplicarla
  // tal cual a un hilo de SMS deja el composer bloqueado sin razón (el
  // composer deriva su `inputsDisabled` de este `expired`). Además solo
  // cuentan los mensajes de WhatsApp: como el hilo mezcla canales, un SMS
  ...
  const sessionInfo = useMemo(() => {
    ...
    const lastCustomerMsg = [...messages]
      .reverse()
      .find(
        (m) => m.sender_type === "customer" && messageChannel(m) === "whatsapp",
      );
  ...
  useEffect(() => {
    ...
    // Ojo con el nombre: aquí `channel` sería el canal de Supabase
    // Realtime, no el de mensajería del hilo. Se llama distinto para que
    // no se confundan al leer.
  ...
  const handleSend = useCallback(
    async (text: string, replyToId?: string) => {
      ...
      const optimisticMsg: MessageWithChannel = {
        id: tempId,
      ...
      try {
        // Cada canal tiene su ruta. Antes TODO salía por /api/whatsapp/send
        // aunque el hilo fuese de SMS: o Meta lo rechazaba con un error
  ...
  // Citar es de WhatsApp: ni el SMS ni el email transportan el
  // `context.message_id` de Meta, así que fuera de ese canal no se abre
  // el borrador de cita en vez de prometer una cita que el cliente no
  // vería. (Ocultar el botón es de MessageActions, que es de otro bloque.)
  const handleStartReply = useCallback(
    (msg: Message) => {
      if (messageChannel(msg) !== "whatsapp") return;
      setReplyTo({
        id: msg.id,
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      // /api/whatsapp/react solo sabe hablar con Meta: reaccionar a un SMS
      // o a un email fallaría en el proveedor. Se corta aquí en vez de
      // dejar que salga la petición.
      const target = messagesById.get(messageId);
      if (target && messageChannel(target) !== "whatsapp") {
        console.warn("[reactions] reactions are WhatsApp-only");
        return;
      }
    ...
    [conversation, messagesById, user?.id],
  );

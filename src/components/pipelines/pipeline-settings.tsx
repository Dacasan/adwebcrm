...
import type { ChecklistItem, Pipeline, PipelineStage } from "@/types";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function SortableStageRow({
  ...
  function moveItem(index: number, dir: -1 | 1) {
    ...
    next[target] = tmp;
    onChecklistChange(next.map((item, i) => ({ ...item, position: i })));
  }
    ...
    >
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {checklist.length > 0 && <span>{checklist.length}</span>}
          <ChevronDown
      ...
      {checklistOpen && (
        <div className="mt-2 space-y-1.5 border-t border-border/60 pt-2">
          ...
          <button
            type="button"
            onClick={addItem}
            className="flex w-full items-center gap-1 rounded-md border border-dashed border-border px-2 py-1.5 text-xs text-muted-foreground hover:border-border hover:bg-muted hover:text-foreground"
          >

...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  emit_conversion: { label: "emit_conversion", icon: Target, border: "border-l-primary" },
}
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType }[] = [
  ...
  // Pipeline de tratos (dispatch server-side). En orden de ciclo de vida del
  // trato: nace (POST /api/deals), se mueve y se cierra (los tres últimos los
  // despacha la ruta de transición, envoltura única de la RPC transition_deal).
  // `deal_won` / `deal_lost` son independientes de `deal_stage_changed`: cerrar
  // un trato arrastrándolo a una etapa terminal dispara los dos, porque son dos
  // hechos distintos —el movimiento y el cierre— y una automatización puede
  // querer reaccionar a uno sin el otro.
  { value: "deal_created" },
...
function blankConfig(type: AutomationStepType): Record<string, unknown> {
  switch (type) {
    ...
    case "emit_conversion":
      // MVP: event_name fijo (qualified_lead). value/currency opcionales.
      return { event_name: "qualified_lead", value: 0 }
    default:
      return {}
  }
}
...
/** Pipeline + etapa de origen y de destino para el disparador
 *  `deal_stage_changed`.
 *
 *  No reutiliza DealPipelineFields porque la semántica es la contraria: allí
 *  la etapa es un DATO que el paso create_deal va a escribir (obligatorio, y
 *  al cambiar de pipeline se autoselecciona la primera), mientras que aquí los
 *  tres campos son FILTROS y "sin valor" significa «cualquiera». Por eso al
 *  cambiar de pipeline se limpian las dos etapas en vez de autoseleccionar:
 *  sus ids pertenecen al pipeline anterior. */
function DealStageTriggerFields({
  ...
  const fromStageId = (config.from_stage_id as string) ?? ""
  const toStageId = (config.to_stage_id as string) ?? ""

  // Se reemplaza el trigger_config entero en vez de hacer spread de `config`:
  // cambiar de tipo de disparador NO limpia el objeto anterior, así que un
  // spread arrastraría aquí el `keywords` o el `tag_id` del disparador previo.
  // Y las claves vacías se omiten en lugar de guardar "": el motor trata la
  // clave presente como filtro a cumplir, y una cadena vacía no casaría nunca.
  const emit = (next: {
    pipeline_id: string
    ...
    if (next.pipeline_id) clean.pipeline_id = next.pipeline_id
    if (next.from_stage_id) clean.from_stage_id = next.from_stage_id
    if (next.to_stage_id) clean.to_stage_id = next.to_stage_id
    onChange(clean)
  }
  ...
  if (pipelines.length === 0) {
    return (
      <>
        ...
        <FieldBlock label={t("pipelines.fromStageIdLabel")}>
          <Input
            value={fromStageId}
            onChange={(e) =>
              emit({
        ...
        <FieldBlock label={t("pipelines.toStageIdLabel")}>
          <Input
            value={toStageId}
            onChange={(e) =>
              emit({
  ...
  const selectedPipeline = pipelines.find((p) => p.id === pipelineId)
  const stageOptions = stages.filter((s) => s.pipeline_id === pipelineId)
  const selectedFromStage = stageOptions.find((s) => s.id === fromStageId)
  const selectedToStage = stageOptions.find((s) => s.id === toStageId)
  // Sin pipeline elegido no hay lista de etapas que ofrecer; el filtro queda
  // en «cualquiera», que es exactamente lo que guarda un config vacío.
  const stagesDisabled = !pipelineId || stageOptions.length === 0
    ...
    <>
      <FieldBlock label={t("pipelines.pipelineLabel")}>
        <select
          ...
          className={SELECT_CLASS}
        >
          <option value="">{t("pipelines.anyPipeline")}</option>
          {pipelines.map((p) => (
            <option key={p.id} value={p.id}>
              {p.name}
            </option>
          ))}
          {/* Un pipeline borrado no debe desaparecer en silencio del config. */}
          {pipelineId && !selectedPipeline && (
            <option value={pipelineId}>{t("pipelines.unknownPipeline", { id: pipelineId })}</option>
          )}
        </select>
      </FieldBlock>
      <FieldBlock label={t("pipelines.fromStageLabel")}>
        <select
          value={fromStageId}
          onChange={(e) =>
            emit({
        ...
        >
          <option value="">
            {pipelineId ? t("pipelines.anyStage") : t("pipelines.selectPipelineFirst")}
          </option>
          {stageOptions.map((s) => (
            <option key={s.id} value={s.id}>
              {s.name}
          ...
          {fromStageId && pipelineId && !selectedFromStage && (
            <option value={fromStageId}>{t("pipelines.unknownStage", { id: fromStageId })}</option>
          )}
        </select>
      </FieldBlock>
      <FieldBlock label={t("pipelines.toStageLabel")}>
        <select
          value={toStageId}
          onChange={(e) =>
            emit({
        ...
        >
          ...
          {toStageId && pipelineId && !selectedToStage && (
            <option value={toStageId}>{t("pipelines.unknownStage", { id: toStageId })}</option>
          )}
        </select>
      </FieldBlock>
    </>
...
/** Filtro de pipeline (opcional) para los disparadores de ciclo de vida del
 *  trato: `deal_created`, `deal_won` y `deal_lost`.
 *
 *  Los tres comparten `DealTriggerConfig`, que solo tiene `pipeline_id`, y por
 *  eso comparten un único componente en vez de tener uno cada uno: la etapa no
 *  es un filtro útil aquí —nacer, ganar y perder son hechos del trato, no del
 *  sitio del tablero donde queda— así que ofrecer un select de etapa sería
 *  prometer un filtro que el motor no aplica.
 *
 *  Es `DealStageTriggerFields` recortado a un solo campo, con la misma
 *  semántica de filtro: sin valor = «cualquier pipeline». */
function DealTriggerFields({
  config,
  onChange,
  t,
}: {
  ...
  // Mismo criterio que DealStageTriggerFields: se reemplaza el trigger_config
  // entero (cambiar de tipo de disparador NO limpia el objeto anterior, y un
  // spread arrastraría el `keywords` o el `to_stage_id` del disparador previo)
  // y la clave vacía se OMITE en vez de guardarse como "": el motor trata la
  // clave presente como filtro a cumplir y "" no casaría con ningún pipeline.
  const emit = (nextPipelineId: string) => {
    onChange(nextPipelineId ? { pipeline_id: nextPipelineId } : {})
  }

  // Invariante del builder: toda automatización debe poder autorarse aunque la
  // lista de recursos venga vacía (cuenta recién creada o RLS sin pipelines).
  if (pipelines.length === 0) {
    return (
      <FieldBlock label={t("pipelines.pipelineIdLabel")}>
        <Input
        ...
        {open && (
          <div className="space-y-3 border-t border-border px-4 py-3">
            ...
            {type === "deal_stage_changed" && (
              <DealStageTriggerFields config={config} onChange={onConfigChange} t={t} />
            )}
            {(type === "deal_created" || type === "deal_won" || type === "deal_lost") && (
              <DealTriggerFields config={config} onChange={onConfigChange} t={t} />
            )}
            {type === "time_based" && (
              <div>
                <label className="mb-1 block text-xs font-medium text-muted-foreground">
...
function StepEditor({
  ...
  switch (step.step_type) {
    ...
    case "emit_conversion":
      // MVP: event_name es literal fijo ('qualified_lead') — no se pide al
      // usuario. value/currency opcionales para reporting interno; Meta
      // recibe el evento QualifiedLead sin custom_data (solo Purchase lo
      // lleva — meta-capi.ts).
      return (
        <>
          <FieldBlock label={t("config.emitValueLabel")}>
            <Input
              type="number"
              value={(cfg.value as number) ?? 0}
              onChange={(e) => set({ value: Number(e.target.value) })}
          ...
          <FieldBlock label={t("config.emitCurrencyLabel")}>
            <Input
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case "emit_conversion": {
      const v = step.step_config.value as number | undefined
      return v ? `qualified_lead · ${v}` : "qualified_lead"
    }
    default:
      return ""
  }

...
/**
 * Paso 4 del wizard de campaña de email: nombre, resumen de audiencia y
 * confirmación de envío. Espejo de `Step4ScheduleSend` (broadcast) pero
 * sin el selector de idioma (los templates de email no lo tienen).
 */
export function Step4EmailSend({
  name,
  onNameChange,
  template,
  audience,
    ...
    async function calculateReach() {
      ...
      try {
        ...
        if (audience.type === 'all') {
          ...
          // For custom-field audiences we resolve with the same resolver
          // the send uses, so the estimate matches actual recipients. We
          // query distinct contacts here without materialising rows.
          const { data } = await supabase
            .from('contacts')
            .select('id');
          setEstimatedReach((data ?? []).length);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Campaign Name */}
      <div>
        <label className="mb-1.5 block text-sm font-medium text-foreground">{t('scheduleSend.campaignName')}</label>
        <Input
          value={name}
          onChange={(e) => onNameChange(e.target.value)}
          placeholder={t('scheduleSend.campaignNamePlaceholder')}
      ...
      <div className="rounded-xl border border-border bg-card/50 p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <p className="truncate text-foreground">{template.name}</p>
          </div>
          <div>

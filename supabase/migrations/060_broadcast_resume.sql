-- ============================================================
-- 060_broadcast_resume
--
-- PROCEDENCIA: viene de upstream (ArnasDon/wacrm) como 038. Renumerada al
-- fusionar: este fork ya tenía su propia 038 (telnyx_config). Va después de
-- la 059, que es de donde sale la versión previa de
-- create_broadcast_with_recipients que esta reemplaza — ese orden hay que
-- conservarlo.
--
...
COMMENT ON COLUMN broadcast_recipients.template_params IS
  'Positional body values for this recipient''s template send ({{1}}, {{2}}, ...), frozen when the broadcast was planned. NULL on rows created before migration 060; a resume treats that as no params.';
...
COMMENT ON COLUMN broadcasts.delivery_locked_at IS
  'Set while a server-side delivery pass is fanning out; NULL when idle. See 060_broadcast_resume.sql.';
